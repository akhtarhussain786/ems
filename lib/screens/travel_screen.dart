...
class _TravelScreenState extends State<TravelScreen> with SingleTickerProviderStateMixin {
  ...
  String _travelDate = DateTime.now().toIso8601String().split('T')[0];
  File? _photo;
  ...
  Future<void> _fetch() async {
    ...
    try {
      final res = await ApiService().get('travel/my');
  ...
  Future<void> _submit() async {
    ...
    try {
      final fields = {
        ...
        'start_km': _startKmCtrl.text,
        'start_location': _startLocationCtrl.text,
      };
      final res = await ApiService().postMultipart('travel/create', fields, _photo);
  ...
  List<dynamic> _getFilteredTravels() {
    if (_filterIndex == 0) return _travels;
    final statusMap = {
      1: 'pending',
      2: 'approved',
      3: 'rejected',
    ...
    return _travels.where((t) => (t['status'] ?? 'pending') == status).toList();
  }
              ...
              child: RefreshIndicator(
                ...
                child: filteredTravels.isEmpty
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      final t = filteredTravels[i];
                      return _buildGlassTravelCard(t);
                    },
                  ),
                ),
              ),
  ...
  Widget _buildEmptyState() {
    String message = 'No travel requests';
    String subMessage = 'Tap + to create a new travel request';

    switch (_filterIndex) {
      case 1:
        message = 'No pending requests';
        subMessage = 'All requests are approved or rejected';
        break;
      case 2:
        message = 'No approved requests';
        subMessage = 'No travel requests have been approved';
        break;
      case 3:
        message = 'No rejected requests';
        subMessage = 'No travel requests have been rejected';
        break;
    }
  ...
  // ==================== GLASS TRAVEL CARD ====================
  Widget _buildGlassTravelCard(Map<String, dynamic> t) {
  ...
  void _showForm() {
    ...
    _photo = null;
    _travelDate = DateTime.now().toIso8601String().split('T')[0];
                ...
                children: [
                  ...
                  _buildDateField(
                    ...
                    onTap: () async {
                      ...
                      if (d != null) {
                        setLocalState(() {
                          _travelDate = d.toIso8601String().split('T')[0];
                        });
                      }
                    },
                  ),
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      padding: const EdgeInsets.all(14),
                      decoration: BoxDecoration(
                        color: _photo != null ? Colors.green.shade50 : Colors.grey.shade50,
                        borderRadius: const BorderRadius.all(Radius.circular(12)),
                        border: Border.all(
                          color: _photo != null ? Colors.green.shade200 : Colors.grey[300]!,
                          width: 1.5,
                        ),
                      ),
                      child: Row(
                        children: [
                          Icon(
                            ...
                            color: _photo != null ? Colors.green : Colors.grey[500],
                            size: 24,
                          ),
                          const SizedBox(width: 12),
                          Expanded(
                            child: Text(
                              _photo != null
                                  ? '📸 Photo captured: ${_photo!.path.split('/').last}'
                                  : 'Tap to capture starting point photo',
                              style: TextStyle(
                                color: _photo != null ? Colors.green.shade700 : Colors.grey[600],
                                fontSize: 13,
                              ),
                              overflow: TextOverflow.ellipsis,
                            ),
                          ...
                          if (_photo != null)
                            IconButton(
                              icon: Icon(Icons.close_rounded, color: Colors.grey[400]),
                              onPressed: () => setLocalState(() => _photo = null),
