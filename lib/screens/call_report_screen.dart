...
class _CallReportScreenState extends State<CallReportScreen> with SingleTickerProviderStateMixin {
  List<dynamic> _calls = [];
  bool _loading = true;
  ...
  int _filterIndex = 0; // 0: All, 1: Completed, 2: Callback, 3: Not Interested

  final List<String> _filterLabels = ['All', 'Completed', 'Callback', 'Not Interested'];
  ...
  Future<void> _fetch() async {
    ...
    try {
      final res = await ApiService().getMyCallReports();
  ...
  Future<void> _submit() async {
    ...
    try {
      final res = await ApiService().createCallReport({
      ...
      if (mounted) {
        ...
        if (res['success'] == true) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('✅ Call report saved successfully'),
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(res['message'] ?? '❌ Failed to save call report'),
  ...
  List<dynamic> _getFilteredCalls() {
    if (_filterIndex == 0) return _calls;
    final statusMap = {
      1: 'completed',
      2: 'callback',
      3: 'not_interested',
    ...
    return _calls.where((c) => (c['status'] ?? '') == status).toList();
  }
              ...
              child: RefreshIndicator(
                ...
                child: filteredCalls.isEmpty
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      final c = filteredCalls[i];
                      return _buildGlassCallCard(c);
                    },
                  ),
                ),
              ),
  ...
  Widget _buildEmptyState() {
    String message = 'No call reports';
    String subMessage = 'Tap + to log your first call';

    switch (_filterIndex) {
      case 1:
        message = 'No completed calls';
        subMessage = 'Complete a call to see it here';
        break;
      case 2:
        message = 'No callbacks';
        subMessage = 'No calls marked for callback';
        break;
      case 3:
        message = 'No not interested calls';
        subMessage = 'No calls marked as not interested';
        break;
    }
  ...
  // ==================== GLASS CALL CARD ====================
  Widget _buildGlassCallCard(Map<String, dynamic> c) {
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  _buildFormDropdown(
                    value: _callStatus,
                    items: ['completed', 'busy', 'no_answer', 'callback', 'not_interested'],
                    label: 'Status',
                    onChanged: (v) => setLocalState(() => _callStatus = v!),
                    getLabel: (s) => s.replaceAll('_', ' ').toUpperCase(),
                  ),
                  const SizedBox(height: 10),
                  _buildFormField(
